...
export default function Navbar() {
  ...
  return (
    <header className="site-navbar topbar">
      ...
      {open && <nav id="mobile-navigation" className="mobile-nav" aria-label="Mobile navigation">{links.map((link) => <Link onClick={() => setOpenForPath(null)} key={link.href} href={link.href} className={pathname === link.href || (link.href !== "/" && pathname.startsWith(`${link.href}/`)) ? "active" : ""}>{link.label}</Link>)}</nav>}
    </header>
  );
}
